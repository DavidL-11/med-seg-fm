...
class MedSegBench(BaseImageDataset):
    """
    MedSegBench Dataset
    """
    ...
    def __init__(
        ...
        super(MedSegBench, self).__init__(transform=transform)
        # Arguments
        self.size = size
        self.split = split
        ...
        self.n_neg = n_neg
        self.bbsize = bbsize
        self.use_multiprompts = use_multiprompts
        ...
        self.n_datasets = len(self.npz_files)

        dataset_name = self.npz_files[dataset_id]
        self.read_npz(self.folder_path + dataset_name)
        self.dataset_name = dataset_name.replace(f"_{self.size}.npz", "")

        self.color_to_label =self.create_color_to_label_dict()
        self.label_to_color = {v: k for k, v in self.color_to_label.items()}

        # The dict is pretty long so this helper method retrieves it
        self.label_to_id = self.get_label_to_class_id()

        self.id_to_color = self.get_id_to_color()
        self.color_to_id = {v: k for k, v in self.id_to_color.items()}

    def clean_gt(self, gt):
        """
        Clean the ground truth by removing small colors with less than 200 pixels.
        """
        unique_colors = np.unique(gt)
        for color in unique_colors[1:]:  # Skip the background color (0)
            # CHeck how many pixels of that color are in the ground truth
            n_pixels = np.sum(gt == color)
            if n_pixels < 600:
                # If less than 200 pixels, set it to 0 (background)
                gt[gt == color] = 0
                logger.debug(f"Removed color {color} with {n_pixels} pixels from ground truth.")

        return gt

    def read_npz(self, file_path):
        """
        Read a .npz file and return the image and ground truth.
        """
        print(f"Reading dataset from {file_path}")
        npz_file = np.load(file_path)
        self.image_list = npz_file[f"{self.split}_images"]
        self.label_list = npz_file[f"{self.split}_label"]

        self.n_images = len(self.image_list)

    def create_color_to_label_dict(self):
        """
        Create a dictionary mapping colors to labels.
        """
        color_to_label = {}
        for color, label in self.info[self.dataset_name.split("_")[0]]["pixel_labels"].items():
            if color.isdigit():
                color_to_label[int(color)] = label
            elif color == "non-zero":
                for i in range(1, 256):
                    color_to_label[i] = label
    ...
    def __getitem__(self, idx):
        ...
        if idx < 0 or idx >= self.n_images:
            raise IndexError(
                f"This dataset only contains [0, {self.n_images - 1}] images. Index {idx} is out of range."
            )
        img = self.image_list[idx]
        gt = self.label_list[idx]

        # Remove colors with less than 400 pixels in the ground truth 
        # (some GTs are corrupted it seems, especially M2caiseg)
        if "m2caiseg" in self.dataset_name or "bkai-igh" in self.dataset_name:
            gt = self.clean_gt(gt)

        # Multiprompts: Split GT first into its unique colors and then use connectedComponents to generate prompt
        # for each object of that color
        if "BOB" in self.mode:
            prompt = self.prompt_gen.generate_prompt(img, dataset=self, confidence=0.4)
        else:
            if self.use_multiprompts:
        ...
        # # Plot all prompts on the original image
        # img_copy = img.copy()
        ...
        #         color = (0, 255, 0) if 1 in p.label else (255, 0, 0)
        #         cv2.circle(img_copy, (p.point[0][0], p.point[0][1]), 5, color, -1)
        # plt.figure(figsize=(8, 8))
        # plt.imshow(img_copy)
        # plt.axis('off')
        # plt.title(f"Image {idx} with Prompts")
        # plt.savefig(f"image_{self.dataset_name}_{idx}_with_prompts.png")
        ...
        # Add class IDs to the prompt
        for p in prompt:
            p.class_id = self.label_to_id.get(p.class_label, -1)
        ...
        data = {
            "img": img,
            "gt": gt,
            "name": f"{self.dataset_name.split("_")[0]}_{idx}.png",
            "id": idx,
            "prompts": prompt,
        }
...
#     29: Heart Atrium Left
    ...
    def get_label_to_class_id(self):
        """
        Get a dictionary mapping label names to class IDs.
        """
        return {
            "Polyp": 4,
            "Prostate": 20,
